...
const PanicAlertOverlay = ({ panicEvent, onDismiss, eventsOpen, notificationButtonRef }) => {
  ...
  const deviceName =
    devices[panicEvent.deviceId]?.name ??
    t('sharedUnknown');
  const eventType = panicEvent.attributes?.alarm || panicEvent.type;
  const translatedEvent = t(prefixString('alarm', eventType));
