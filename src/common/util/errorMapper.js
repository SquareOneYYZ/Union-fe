const mapError = (rawMessage, status) => {
  if (!rawMessage || rawMessage.trim() === '') {
    ...
    if (status === 404) return null; // silent
  ...
  if (firstLine.includes('is disabled')) return 'This account or device is disabled.';
  if (firstLine.includes('has expired')) return 'This account or device has expired.';
  ...
  if (firstLine.includes('Invalid VIN')) return firstLine;
  ...
  if (firstLine.includes('Query cannot be empty')) return firstLine;
